...
class Tokenizer(object):
    ...
    del number_re
    other_re = r"(?:[^#\s\.]|\.(?!\.))+"
    _token_regexs = {'usernames': usernames_re, 'urls': urls_re,
                     'hashtags':hashtags_re, 'times':times_re,
                     'phonenumbers':phonenumbers_re, 'numbers':numbers_re}
    tokenize_re = re.compile(
        r"|".join(
            map(lambda x: getattr(x, 'pattern', x),
                 [usernames_re, urls_re, hashtags_re, times_re, phonenumbers_re, numbers_re] + [word_re, ellipsis_re, other_re])))
